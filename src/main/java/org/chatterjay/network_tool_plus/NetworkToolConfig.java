...
public final class NetworkToolConfig {
    ...
    static {
        var pair = new ModConfigSpec.Builder().configure(Values::new);
    ...
    private static final class Values {
        private final ModConfigSpec.ConfigValue<List<? extends String>> whitelist;
     ...
     *
     * <p>The config is read on both logical sides because the network tool inventory is
     * also constructed client-side while a menu is open. SERVER-type configs are synced to
     * connecting clients by the loader, so a joining client sees the host's rules rather than
     * its own empty defaults.</p>
     */
    ...
    private static List<? extends String> getEntries(ModConfigSpec.ConfigValue<List<? extends String>> value) {
        try {
